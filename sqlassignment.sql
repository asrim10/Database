...
(4, 'Butwal', 12, 'Rupandehi', '1'),
(5, 'Tansen', 8, 'Palpa', '1');

-- Inserting addresses from Students table
INSERT INTO address (address_id, City, WardNo, District, Province) VALUES
(6, 'Butwal', 10, 'Rupandehi', '1'),
(7, 'Taulihawa', 5, 'Kapilvastu', '1'),
(8, 'Siddharthanagar', 12, 'Rupandehi', '1');
...
SET amount = CONCAT('NPR ', 
    CAST(REPLACE(TRIM(REPLACE(amount, 'NPR ', '')), ',', '') * 1.2 AS DECIMAL(10, 2)))
WHERE Fees_id > 0;
...
insert into Address
Values(9,"Kathmandu",3,"Bhaktapur","2");
insert into Guardians
Values(4,"Zenith Shrestha","9843119895");

insert into Students
Values(104,"Aayush Shrestha","2004-09-23","Male",4,9);
...
-- Insert records for a student who is absent for 10 consecutive days
INSERT INTO Attendance (Attendance_id, student_id, date, status) VALUES 
    (7, 104, '2023-07-01', 'Absent'),
    (8, 104, '2023-07-02', 'Absent'),
    (9, 104, '2023-07-03', 'Absent'),
    (10, 104, '2023-07-04', 'Absent'),
    (11, 104, '2023-07-05', 'Absent'),
    (12, 104, '2023-07-06', 'Absent'),
    (13, 104, '2023-07-07', 'Absent'),
    (14, 104, '2023-07-08', 'Absent'),
    (15, 104, '2023-07-09', 'Absent'),
    (16, 104, '2023-07-10', 'Absent');
select * from Attendance;

-- Delete records for the student absent for 10 consecutive days
DELETE FROM Attendance
WHERE student_id IN (
    SELECT student_id
    FROM (
        SELECT student_id, COUNT(*) AS absence_days
        FROM Attendance
        WHERE status = 'Absent'
        GROUP BY student_id, 
                 DATE_SUB(date, INTERVAL @rn:=@rn+1 DAY)
        HAVING absence_days >= 10
    ) AS subquery
);
...
-- Report of students' total marks in each subject
SELECT 
    r.Student_id,
    s.StudentName,
    r.ExamName,
...
-- Overall percentage of each student
SELECT 
    r.Student_id,
    s.StudentName,
    SUM(r.Score) AS TotalMarks,
    ROUND((SUM(r.Score) * 100.0 / (COUNT(DISTINCT r.ExamName) * 100)), 2) AS OverallPercentage
FROM 
    Results r
JOIN 
    Students s ON r.Student_id = s.Student_id
